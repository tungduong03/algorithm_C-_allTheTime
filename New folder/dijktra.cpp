...
void Dijkstra(void){
 ...
 chuaxet[s] = TRUE;  //đã xét đến đỉnh S

 //bươc lặp

while (!chuaxet[t]) {  //khi chưa xét đến đỉnh cuối

    minp = VOCUNG;

    //tìm đỉnh u sao cho d[u] là nhỏ nhất trong các đỉnh chưa xét
    ...
    chuaxet[u] = TRUE;// u la dinh co nhan tam thoi nho nhat, thăm đỉnh u

    if (!chuaxet[t]){  //nếu vẫn chưa phải đỉnh cuối

        //gán nhãn lại cho các đỉnh(trọng số đường đi, và đỉnh kề trước)
